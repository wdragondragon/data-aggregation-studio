...
- 2026-07-01 已完成 S111 认证上下文 Profile 源头瘦身：确认并修复 `/auth/me` 和登录 Profile 只需全局头部租户、项目、角色上下文，却读取用户、角色、租户、项目整实体的问题，`FIX-S111-001` 已进入缺陷回归索引；保持 `/auth/login`、`/auth/me` 响应结构不变，正常分页拉取、当前页刷新和运行状态刷新不作为问题。
- S111 本轮未新增或清理长期数据；`StudioAccessProfileSourceSlimmingRegressionTest` 2 tests 全部通过，`mvn -pl studio-server -am -DskipTests package` 通过；build-nginx API `POST /auth/login` 与 `GET /auth/me` 返回 `SUCCESS`，长期项目 header 返回 `currentProjectId=2068077680446365698`；build-nginx `/dashboard` 切换到 `Default Tenant / 长期回归测试项目` 后显示正常，console warn/error 为 0。后续继续按“分页该拉取还是拉取刷新状态，重点排查隐藏详情读取、大 JSON 源头读取和跨上下文刷新”执行。
- 2026-07-01 已完成 S112 采集任务上线响应源头瘦身：确认并修复 `/collection-tasks/{id}/online` 列表页上线动作只需当前行摘要，却返回完整采集任务定义和多类详情 JSON 的问题，`FIX-S112-001` 已进入缺陷回归索引；保持采集任务分页、当前页刷新、保存/编辑/详情/预览语义不变。
...
- 2026-07-03 已完成 S119 运行指标与旧运行摘要复核：`/run-metrics` 源头复核未发现新过取，确认 options 走 `collection_task_metric_binding` 摘要、dashboard 走运行记录聚合 SQL；定向回归发现并修复旧运行记录缺少指标列时列表摘要被误推导为 0 的隐性问题，`FIX-S119-001` 已进入缺陷回归索引。
- S119 本轮未新增或清理长期数据；`RunMetricsApiRegressionTest` 单测通过，组合回归 `MetricsSourceSlimmingRegressionTest,CollectionTaskListSourceSlimmingRegressionTest,RunListSourceSlimmingRegressionTest,RunMetricsApiRegressionTest` 共 16 tests 全部通过；`mvn -pl studio-server -am -DskipTests package` 通过，重启 server PID `15628` 后 health 为 `UP`，build-nginx `/run-metrics` 与 `/collection-task-runs` 长期项目页面 console warn/error 为 0。后续继续保留分页当前页拉取、刷新按钮和状态刷新，重点排查详情大字段、完整日志和跨上下文数据读取。
- 2026-07-03 已完成 S120 采集运行日志页源头复核：`/collection-task-runs` 代码与 build-nginx 页面均复核通过，未发现需要修复的真实过取问题；采集任务筛选下拉只取 `id/projectId/name`，运行列表字段级分页，日志摘要和日志内容按 id 独立下钻。
- S120 本轮未新增或清理长期数据；`RunListSourceSlimmingRegressionTest` 与 `CollectionTaskListSourceSlimmingRegressionTest` 共 10 tests 全部通过；build-nginx `/collection-task-runs` 显示长期项目上下文、当前页 10 条、总数 119，日志抽屉可用且 console warn/error 为 0。后续继续按“分页该拉取还是拉取刷新状态”，只把列表入口读取详情大字段、完整日志、完整定义 JSON 或跨上下文无关数据判为问题。
- 2026-07-03 已完成 S121 质量运行分页名称水合源头瘦身：确认并修复 `/runs/page` 作为 `/quality-task-runs` 和 `/collection-task-runs` 表格接口时，字段级查询已经轻量但名称水合仍读取全项目、跨资源类型名称的问题，`FIX-S121-001` 已进入缺陷回归索引。
- S121 本轮未新增或清理长期数据；`RunListSourceSlimmingRegressionTest`、`CollectionTaskListSourceSlimmingRegressionTest`、`QualityListSourceSlimmingRegressionTest` 共 18 tests 全部通过，`mvn -pl studio-server -am -DskipTests package` 通过；重启 server PID `4436` 后 health 为 `UP`，build-nginx `/quality-task-runs` 显示长期项目、总数 87、日志抽屉可用且 console warn/error 为 0；nginx API 验证质量运行不再返回采集任务名/工作流名，采集运行不再返回质量任务名/工作流名。
- 2026-07-03 已完成 S122 数据服务与协议转换列表发布下线响应源头瘦身：确认并修复 `/data-services/{id}/publish`、`/offline` 与 `/protocol-conversions/{id}/publish`、`/offline` 在列表页动作中返回完整定义视图的问题，`FIX-S122-001` 已进入缺陷回归索引；原完整接口保留给详情/编辑场景。
- S122 本轮未新增或清理长期数据，复用长期数据服务 `长期回归-S12客户边界大结果集查询服务` / `2068717536226799617` 和协议转换服务 `长期回归-S15客户SOAP协议转换脱敏服务` / `2068748392102285314`；三类列表源头瘦身回归 10 tests 全部通过，`npm run build:web`、`mvn -pl studio-server -am -DskipTests package` 均通过，重启 server 后 health 为 `UP`；build-nginx API summary 响应不含详情配置字段，`/data-services` 显示 4 条、`/protocol-conversions` 显示 3 条，console warn/error 为 0。后续继续保留分页当前页拉取、写后重拉列表和状态刷新，只把列表动作返回完整定义、读取详情大字段或跨上下文无关数据判为问题。
- 2026-07-03 已完成 S123 质量规则列表启停响应源头瘦身：确认并修复 `/quality-rules/{id}/enable`、`/disable` 在列表页动作中返回完整质量规则视图的问题，`FIX-S123-001` 已进入缺陷回归索引；原完整接口保留给需要完整规则视图的调用方。
- S123 本轮未新增或清理长期数据，复用质量规则 `长期回归-S10合同金额完整性规则` / `2068704763174420481`，API 往返后已恢复原启用状态；`QualityListSourceSlimmingRegressionTest` 8 tests 全部通过，`npm run build:web`、`mvn -pl studio-server -am -DskipTests package` 均通过，重启 server PID `10216` 后 health 为 `UP`；build-nginx API summary 响应不含 `description/supportedDatasourceTypes/logicSql/inputParams/outputParams`，`/quality-rules` 显示长期项目、规则总数 21，console warn/error 为 0。后续继续保留分页当前页拉取、刷新按钮、写后重拉当前页和状态刷新，只把列表动作返回完整定义、写前读取详情大字段或整实体回写判为问题。
- 2026-07-03 已完成 S124 质量问题处置写接口源头瘦身：确认并修复 `/quality-metrics/issues/{id}/assign`、`/status`、`/severity`、`/comment` 写动作前置读取完整问题实体、状态/负责人整实体回写的问题，`FIX-S124-001` 已进入缺陷回归索引；详情抽屉写后返回完整详情保留。
- S124 本轮保留长期评论：质量问题 `QI-2068704862793318402` 追加 `长期回归-S124：验证质量问题处置写接口轻量前置读取与详情返回。`；`MetricsSourceSlimmingRegressionTest` 6 tests 全部通过，`mvn -pl studio-server -am -DskipTests package` 通过，重启 server PID `1624` 后 health 为 `UP`；build-nginx API `/quality-metrics/issues/page` 返回长期问题总数 3，`/comment` 返回详情和时间线；build-nginx `/quality-metrics` 问题中心 Tab 显示长期项目，console warn/error 为 0。后续继续保留分页、筛选、详情抽屉按 id 加载和写后详情返回，只把写前读取详情大字段或整实体回写判为问题。
- 2026-07-03 已完成 S125 能力矩阵源头复核：`/catalog` 页面与 `/catalog/capabilities` 接口代码、API 和 build-nginx 均复核通过，未发现需要修复的真实过取问题；页面需要整张 18 行能力配置矩阵、读写统计和可执行类型徽标，刷新按钮重取同一配置接口属于正常行为。
- S125 本轮未新增或清理长期数据；`StudioInitializationApiRegressionTest#catalogEndpointsShouldExposeTableDrivenDatasourceTypesAndCapabilities` 1 test 通过，build-nginx API `/catalog/capabilities` 返回 `sourceCapabilities=18`、`readable=13`、`writable=11`、`executableDatasourceTypes=18`，包含 `mysql8/http`；build-nginx `/catalog` 显示长期项目上下文和能力矩阵。后续继续按“分页该拉取还是拉取刷新状态”执行，只把列表入口读取详情级大字段、完整定义 JSON、跨上下文无关数据或写前整实体读取/回写判为问题。
- 2026-07-03 已完成 S126 元模型同步写接口源头瘦身：确认并修复 `/metadata` 技术元模型同步写动作返回所有字段定义、后端同步前通过全量元模型字段列表查找目标技术元模型的问题，`FIX-S126-001` 已进入缺陷回归索引；原完整同步接口保留，页面改用新增 summary 接口。
- S126 本轮保留一次 `mysql8` 技术元模型 summary 同步长期写入；`MetadataSchemaSourceSlimmingRegressionTest` 与 `StudioInitializationApiRegressionTest#metadataSchemasShouldContainRequiredMysqlTechnicalMetaModelsAndFieldDefinitions` 共 2 tests 全部通过，`mvn -pl studio-server -am -DskipTests package` 与 `npm run build:web` 通过，重启 server PID `21648` 后 health 为 `UP`；build-nginx API summary 返回 3 条且字段计数 `0,0,0`，摘要列表 82 个元模型均无字段，按 id 详情仍返回 mysql8 表元模型 11 个字段；build-nginx `/metadata` 显示长期项目上下文，console warn/error 为 0。
- 2026-07-03 已完成 S127 数据源中心源头复核：`/datasources` 页面、代码和 build-nginx API 均复核通过，未发现需要修复的真实过取问题；分页当前页拉取、刷新按钮、连接状态刷新、编辑详情按 id 下钻、连接历史弹窗按需读取均按正常行为处理。
- S127 本轮未新增、未修改、未清理长期数据；`DataSourceListSourceSlimmingRegressionTest` 与 `DataSourceDiscoveryOptionsSourceSlimmingRegressionTest` 共 9 tests 全部通过；build-nginx API `/datasources/page` 返回长期项目 6 条且不含 `technicalMetadata/businessMetadata`，`/datasources/options` 返回 6 条且不含连接健康和元数据，`/meta-schemas?includeFields=false` 返回 82 个摘要且字段计数为 0；build-nginx `/datasources` 列表、刷新、编辑抽屉、连接历史弹窗均可达，console warn/error 为 0。
