...
| BUG-M05-001 | 数据接入 | 高 | MySQL writer 默认 insert 模式实际表现为 upsert | `backend/studio-infra/src/main/java/com/jdragon/studio/infra/service/DataIngestionExecutionSupport.java`；DataAggregation writer 相关代码 | 接入服务重复主键写入应失败，不应 upsert | smoke | 2026-06-20 | `b59e8d5` |
| BUG-M05-002 | 数据接入 | 高 | 数据接入写入失败时开放 API 暴露 Java 堆栈和内部路径 | `backend/studio-infra/src/main/java/com/jdragon/studio/infra/service/DataIngestionExecutionSupport.java`；`GlobalExceptionHandler` | `DataIngestionInvocationLogSupportTest`；开放 API 错误提示不含内部路径/堆栈 | smoke | 2026-06-20 | `b59e8d5` |
