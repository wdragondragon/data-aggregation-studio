...
- Batch 19 继续拆分质量任务编辑页：新增参数绑定、告警配置、调度配置和校验结果 section 组件，`QualityTaskEditorView.vue` 降到 1016 行。
- Batch 20 继续拆分质量任务编辑页：新增基本信息、任务绑定、规则选择和 SQL 预览 section 组件，`QualityTaskEditorView.vue` 降到 884 行。
- Batch 21 拆分工作流资源选择弹窗：新增 `WorkflowResourceDialogs.vue`，将采集任务、质量任务和脚本选择/预览弹窗从 `WorkflowEditorView.vue` 移出，页面降到 1234 行。
- Batch 22 拆分采集任务调度与预览区块：新增 `CollectionTaskScheduleSection.vue` 和 `CollectionTaskReviewSection.vue`，将调度配置与最终预览从 `CollectionTaskEditorView.vue` 移出，页面降到 1355 行。
- Batch 23 拆分采集任务源端与目标端绑定区块：新增 `CollectionTaskBindingSection.vue`，将任务基础信息、源端/目标端绑定、reader/writer 高级参数和增量游标展示从 `CollectionTaskEditorView.vue` 移出，页面降到 1122 行。
...
- `npm run build:web`（`frontend` 目录）：通过，`vue-tsc --noEmit && vite build` 成功；Batch 19 质量任务参数、告警、调度和校验结果 section 拆分后验证。
- `npm run build:web`（`frontend` 目录）：最终通过，`vue-tsc --noEmit && vite build` 成功；Batch 20 质量任务基础表单、绑定、规则选择和 SQL 预览 section 拆分后验证，中间曾因 Vue ref 回调类型过窄失败并已修复。
- `git diff --check`：Batch 20 首次失败，原因是 `QualityTaskEditorView.vue` 文件末尾多余空行；修复后通过，仅有 Windows 工作区 LF/CRLF 替换提示。
- `npm run build:web`（`frontend` 目录）：通过，`vue-tsc --noEmit && vite build` 成功；Batch 21 工作流资源选择弹窗拆分后验证。
- `npm run build:web`（`frontend` 目录）：通过，`vue-tsc --noEmit && vite build` 成功；Batch 22 采集任务调度与预览区块拆分后验证。
- `npm run build:web`（`frontend` 目录）：通过，`vue-tsc --noEmit && vite build` 成功；Batch 23 采集任务源端与目标端绑定区块拆分后验证。
...
- Batch 19：质量任务编辑页主要表格/结果 section 已独立；后续可继续拆基本信息、绑定、规则选择和 SQL 预览区，或转向工作流/采集任务编辑器。
- Batch 20：质量任务编辑页已降到 1000 行以下，后续不建议继续为行数硬拆；若继续治理，应抽保存/预览/校验 composable 或转向工作流、采集任务、模型中心等仍较大的页面。
- Batch 21：工作流资源选择弹窗已独立；工作流编辑页剩余主要债务在节点配置面板，后续可拆 HTTP 节点参数表与不同节点类型配置区块。
- Batch 22：采集任务编辑页调度和预览 section 已独立；主要复杂度仍在源端/目标端绑定和运行参数、字段映射/融合配置，后续优先拆这些区块。
- Batch 23：采集任务源端/目标端绑定 section 已独立；后续优先拆第 2 步字段映射/融合配置，以及父页面中的运行参数 schema 和自定义 SQL 字段解析 composable。
