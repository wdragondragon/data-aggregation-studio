...
- Batch 5/11/12：继续前端大页面治理，`RunsView` 已接入 `usePageQuery`，`MetadataSchemasView` 和 `ModelStatisticsView` 已接入 `useAsyncAction`；后续优先拆采集任务编辑、质量任务编辑、工作流编辑、模型中心为 composable + section components。
