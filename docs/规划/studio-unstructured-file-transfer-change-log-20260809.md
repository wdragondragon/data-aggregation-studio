...
| W6 | 后端、前端、真实 OSS/FTP、重启和健康检查验收 | 已完成 | 2026-08-09 | 真实双向传输、文件操作、ACL、队列、指标、SSE、构建和健康检查通过 |
...
完整命令、测试类、真实运行证据、页面交互和残余风险记录在 [`studio-file-transfer-acceptance-record-20260807.md`](../测试/文件传输/studio-file-transfer-acceptance-record-20260807.md)。当前聚焦测试、真实 OSS/FTP 和浏览器验收均通过；本次没有把未执行的 Studio 全仓非文件传输测试表述为绿色。
...
单集群模型、历史兼容、非结构化管理、ACL、前端交互、数据库三套结构、定向测试、前端构建、真实 OSS/FTP、接口/部署/测试文档、`git diff --check` 以及 Server/Worker/Frontend 重启健康检查均已完成。10 GiB、SFTP/MinIO 和生产反向代理属于已记录的部署环境扩展验收。
