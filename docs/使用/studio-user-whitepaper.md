...
统一告警中心位于“运维中心 / 告警中心”，用于把任务、服务、Worker、队列和日志异常聚合为项目级事件，并通过站内信或 Webhook 通知接收人。

推荐操作顺序：

1. 需要外部通知时先创建并测试 Webhook；只使用站内信时可跳过。
2. 创建规则，选择单个资源或“全部匹配对象”，配置阈值、静默时间和接收人。
3. 先保持规则停用，执行规则测试，确认投递成功后再启用。
4. 在“告警事件”中查看证据和时间线，确认或关闭事件。
5. 在“通知通道”的投递记录中查看 `RETRY`、`DEAD`、`SKIPPED` 并由管理员人工重试。
