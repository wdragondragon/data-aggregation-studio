...
| 运行监控与告警 | 查看任务日志、服务调用日志、异常聚合，配置项目级告警并完成确认、恢复和通知闭环 | 运维与值班人员 |
...
| 运维中心 | 统一运维中心、告警中心 | 查看运行、队列、Worker、日志健康状态，维护告警规则和通知闭环 |

![图 3-2 工作台总览与左侧菜单](images/studio-dashboard.jpg)

### 3.4 模块文档索引

本白皮书提供 Studio 的综合使用说明。需要按具体功能查阅操作细节时，可进入 [功能模块文档](./功能模块/README.md)：

| 模块 | 文档 |
|---|---|
| 工作台与项目上下文 | [01-工作台与项目上下文.md](./功能模块/01-工作台与项目上下文.md) |
| 数据源管理 | [02-数据源管理.md](./功能模块/02-数据源管理.md) |
| 模型中心与模型同步 | [03-模型中心与模型同步.md](./功能模块/03-模型中心与模型同步.md) |
| 数据采集任务 | [04-数据采集任务.md](./功能模块/04-数据采集任务.md) |
| 数据开发 | [05-数据开发.md](./功能模块/05-数据开发.md) |
| 工作流编排 | [06-工作流编排.md](./功能模块/06-工作流编排.md) |
| 数据服务 | [07-数据服务.md](./功能模块/07-数据服务.md) |
| 数据接入服务 | [08-数据接入服务.md](./功能模块/08-数据接入服务.md) |
| 协议转换服务 | [09-协议转换服务.md](./功能模块/09-协议转换服务.md) |
| 数据质量 | [10-数据质量.md](./功能模块/10-数据质量.md) |
| 系统管理 | [11-系统管理.md](./功能模块/11-系统管理.md) |
| 运维中心与日志 | [12-运维中心与日志.md](./功能模块/12-运维中心与日志.md) |
| 统一告警中心 | [13-统一告警中心.md](./功能模块/13-统一告警中心.md) |
...
统一告警中心位于“运维中心 / 告警中心”，用于把任务、服务、Worker、队列和日志异常聚合为项目级事件，并通过站内信、Webhook 或 eLink 通知接收人。eLink 跟随规则接收人时优先使用账号绑定，未绑定时尝试使用 Studio 用户手机号码。

推荐操作顺序：

1. 需要外部通知时先创建并测试 Webhook 或 eLink 通道；只使用站内信时可跳过。
2. 创建规则，选择单个资源或“全部匹配对象”，配置阈值、静默时间和接收人。
3. 先保持规则停用，执行规则测试，确认投递成功后再启用。
4. 在“告警事件”中查看证据和时间线，确认或关闭事件。
5. 在“通知通道”的投递记录中查看触发规则、告警对象、实际发送内容、接收目标和 `RETRY / DEAD / SKIPPED` 状态，并由管理员人工重试。

首版支持执行失败、连续失败、运行超时、服务失败率、接入或转换调用失败、Worker 离线、队列积压、调度延迟和日志上传失败九类规则。项目成员可查看、确认和关闭事件；项目管理员、租户管理员和超级管理员可维护规则、通道和投递重试。

状态流转为 `OPEN / ACKNOWLEDGED / RECOVERED / CLOSED`。如果在异常仍成立时人工关闭，系统不会在下一轮评估中立即重开；必须先观察到一次恢复，之后再次异常才会产生 `REOPENED`。

完整的九类配置例子、页面截图、权限表、状态机、Webhook 安全边界和排障方法见 [统一告警中心使用指南](./功能模块/13-统一告警中心.md)。
