...
非结构化管理页面的多选删除继续复用上述单项操作接口，不新增批量接口。页面冻结用户确认时的集群、数据源和选择项，经过一次二次确认后按选择顺序逐项提交 `DELETE`；目录项设置 `recursiveConfirmed=true`。单项失败不会阻止后续条目，全部请求结束后目录只刷新一次，并分别提示成功数、失败数及有限的失败摘要，因此该交互不是原子批量事务。
