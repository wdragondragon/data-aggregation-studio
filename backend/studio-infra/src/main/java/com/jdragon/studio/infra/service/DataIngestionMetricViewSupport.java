...
final class DataIngestionMetricViewSupport {

    DataIngestionAccessLogView toAccessLogView(DataIngestionAccessLogEntity entity) {
        DataIngestionAccessLogView view = new DataIngestionAccessLogView();
    ...
    Map<Long, DataIngestionServiceEntity> toServiceMap(List<DataIngestionServiceEntity> services) {
        Map<Long, DataIngestionServiceEntity> map = new LinkedHashMap<Long, DataIngestionServiceEntity>();
    ...
    String firstServiceName(List<DataIngestionAccessLogEntity> logs) {
        if (logs == null) {
            return absent();
        }
        for (DataIngestionAccessLogEntity log : logs) {
            if (hasText(log.getServiceNameSnapshot())) {
                return log.getServiceNameSnapshot();
            }
        }
        return absent();
    }

    String firstServiceCode(List<DataIngestionAccessLogEntity> logs) {
        if (logs == null) {
            return absent();
        }
        for (DataIngestionAccessLogEntity log : logs) {
    ...
    String firstServiceStatus(List<DataIngestionAccessLogEntity> logs) {
        if (logs == null) {
            return absent();
        }
        for (DataIngestionAccessLogEntity log : logs) {
