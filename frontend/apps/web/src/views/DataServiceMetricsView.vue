<template>
  <div class="studio-page data-service-metrics-page">
    ...
    <SectionCard title="API 统计" description="按服务聚合访问次数、成功率、响应时间和缓存命中率。">
      <StudioTableShell v-loading="apiStatsLoading" min-width="1540px">
...
async function loadOptions() {
  const payload = await studioApi.dataServiceMetrics.options(LOCAL_LOADING_REQUEST);
...
async function loadDashboard() {
  ...
  try {
    dashboard.value = await studioApi.dataServiceMetrics.queryDashboard(buildQuery(), LOCAL_LOADING_REQUEST);
  } catch (error) {
...
async function loadApiStats() {
  ...
  try {
    ...
    ElMessage.error(error instanceof Error ? error.message : "加载 API 统计失败");
  } finally {
